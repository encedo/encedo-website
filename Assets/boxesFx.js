...
;( function( window ) {
	...
	BoxesFx.prototype._init = function() {
		...
		this._setTransforms();
		// which effect
		this.effect = 'effect-' + parseInt((Math.random()*7498324329)%3+1);
		// check if animating
		this.isAnimating = false;
		// the panels
		this.els = this.el.querySelectorAll( '.panel' );
		this.panels = (this.els ? [].slice.call( this.els ) : []);
	...
	BoxesFx.prototype._navigate = function( dir ) {
		...
		var cntTransTotal = 0,
			...
			onEndTransitionFn = function( ev ) {
				...
				self.effect = 'effect-' + parseInt((Math.random()*7498324329)%3+1);
			};
